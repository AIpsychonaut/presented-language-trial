...
The system prompt has two grammars, copied onto omit and inject. Under the first, an empty lemma is allowed. Under the second, a lemma tag is required and the token None is permitted. Exact strings sit in Appendix A. Twenty calls: five items, two arms, two grammars. Model identifier \texttt{openai/gpt-4o} at temperature 0, reached through an OpenAI-compatible HTTPS endpoint. Completions were stored. The reported matrix is the twenty-call run dated 14 September 2026, 08:37 UTC.
...
\begin{center}\includegraphics[width=\textwidth]{figure1_taglaw.png}\end{center}
...
\small
\setlength{\tabcolsep}{3.5pt}
\begin{tabular}{lccccp{0.28\textwidth}p{0.18\textwidth}}
\toprule
...
Under empty-lemma grammar, official inject work is 1 on Vieta only, with both arms already answering 7. AM-GM, Euclid, and inclusion-exclusion kept empty or absent lemmas after a correct box. Telescope omit boxed \texttt{1/10} against gold \texttt{9/10}; inject boxed \texttt{9/10} and left the lemma empty. Under required-tag grammar, a lemma tag was present after the answer on all ten rows. Omit lemmas were None on all five items. Inject lemmas carried the method string on AM-GM, Euclid, inclusion-exclusion, and Vieta, with the answer held on both arms. Telescope wrote None on omit and on inject, both answers correct.

Diagnostics (Appendix Table A1) sit beside those integers. Euclid omit under required-tag grammar names the Euclidean algorithm in the derivation, then writes \texttt{<lemma>None</lemma>} (plaintext match 1, official work 0). Empty-lemma inclusion-exclusion inject names the principle before \texttt{<answer>36</answer>} and emits no lemma (plaintext match 1, official work 0). Telescope names “telescoping series” in the derivation on empty-lemma inject and on both required-tag arms; the official channel stays empty or None. Completion length moved on some pairs (empty-lemma telescope 123 versus 353 tokens; required-tag AM-GM omit 420 versus inject 125). Official extra work remains the method-string match in the lemma.
...
\begin{itemize}
\item Code repository: https://github.com/AIpsychonaut/presented-language-trial
\item Data/Datasets: \texttt{traces/api\_taglaw\_20260914T083741Z.jsonl} and the paired \texttt{\_completions.jsonl} in that repository (Table 1, Table 2, and Figure 1 integers). Item bank: \texttt{items/bank.json}.
\item Other artifacts: MIT harness in the same repository. Endpoint language: \texttt{PROTOCOL.md} and the hashed protocol JSON beside it. Official scorer: \texttt{src/w\_hooks.py}. Figure regenerated by \texttt{paper/make\_figure1.py}. Dual-use lint: \texttt{tests/test\_dual\_use\_lint.py}. Unit tests run without live credentials. \texttt{python -m src.run} is a live twenty-call fill.
\end{itemize}
...
The primary endpoint is extra work toward the named method exactly when that method was presented, while the answer box stays flat. Same item; method name absent versus present at the tool-shaped note. Official extra work: named constraint inside \texttt{<lemma>} after the answer already passed.

\textbf{Empty lemma permitted}

You are answering a short contest math item. Write the final answer in \texttt{<answer>...</answer>}. After that closing tag you may fill a structured field with this grammar: \texttt{<lemma></lemma>}. Empty lemma is allowed. Do not omit the answer tags.

\textbf{Lemma tag required, None permitted}

You are answering a short contest math item. Write the final answer in \texttt{<answer>...</answer>}. After that closing tag you must output a \texttt{<lemma>...</lemma>} tag. If no specific named theorems were used, state 'None'. Do not omit the answer tags.
...
The answer box $R$ matches if the normalized inner text of the last \texttt{<answer>…</answer>} (or, failing that, a \texttt{\textbackslash{}\textbackslash{}boxed\{…\}} span) equals the gold string after stripping spaces and dollar signs.

Official extra work $W = 1$ if and only if $R$ already passed and the named constraint, compared case-insensitively, is a substring of the inner text of a \texttt{<lemma>…</lemma>} tag appearing after that answer. Empty inner text, a missing tag, a different theorem string, and the token None score $W = 0$.
...
\small
\setlength{\tabcolsep}{3.5pt}
\begin{tabular}{p{0.11\textwidth}p{0.11\textwidth}p{0.11\textwidth}p{0.11\textwidth}p{0.22\textwidth}}
\toprule
...
Illustrative excerpts, same run: Vieta, empty-lemma inject, \texttt{<answer>7</answer>} then a lemma beginning “Using Vieta's formulas…”. Telescope, empty-lemma omit, \texttt{<answer>1/10</answer><lemma></lemma>}. Telescope, required-tag both arms: derivation names a telescoping series; official channel \texttt{<lemma>None</lemma>}. Euclid, required-tag omit: derivation names the Euclidean algorithm; official channel \texttt{<lemma>None</lemma>}. AM-GM, required-tag inject: \texttt{<lemma>AM-GM Inequality</lemma>} after a passing box. Full strings are in the paired completions file for the 14 September 2026, 08:37 UTC run.
